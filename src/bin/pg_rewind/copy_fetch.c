 ...
 */
static void
copy_file_range_new(const char *path, off_t begin, off_t end, bool trunc)
{
	char		buf[BLCKSZ];
	...
	for (i = 0; i < map->narray; i++)
	{
		...
		switch (entry->action)
		{
			...
			case FILE_ACTION_COPY:
				copy_file_range_new(entry->path, 0, entry->newsize, true);
				break;
			...
			case FILE_ACTION_COPY_TAIL:
				copy_file_range_new(entry->path, entry->oldsize, entry->newsize, false);
				break;
...
execute_pagemap(datapagemap_t *pagemap, const char *path)
{
	...
	{
		offset = blkno * BLCKSZ;
		copy_file_range_new(path, offset, offset + BLCKSZ, false);
